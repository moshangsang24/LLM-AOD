...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    F2 = np.random.uniform(0.3, 0.9, dim)
    ...
    mutant += 0.05 * levy * (self.ub - self.lb)
    
    # Apply adaptive polynomial mutation
    p_m = max(0.05, min(0.2, 1 - (self.cur_gen / self.max_gen)**1.2))
    eta_m = 20 + 20 * np.sqrt(self.cur_gen / self.max_gen)
    for j in range(dim):
    ...
    p_obl = max(0.05, min(0.2, 1 - np.sqrt(self.cur_gen / self.max_gen)))
    ...
    if np.random.random() < 0.15:
        local_step = np.random.uniform(-0.03, 0.03, dim) * (self.ub - self.lb)
        mutant += local_step
