...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    F3 = np.clip(np.random.normal(0.3, 0.2), 0.1, 1.0)
    ...
    levy = np.random.standard_cauchy(dim) * 0.1 * (1 - self.cur_gen / self.max_gen)
    mutant += levy
    ...
    # Local search around the best solution with adaptive step size
    if np.random.random() < 0.2:
        step_size = 0.01 * (1 - self.cur_gen / self.max_gen)
        local_step = np.random.normal(0, step_size, dim)
