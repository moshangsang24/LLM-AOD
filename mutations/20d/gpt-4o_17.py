...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    # Combination of Gaussian, Uniform, Levy flight, Differential Evolution (DE) perturbations, Simulated Annealing (SA), Swarm Intelligence, and Adaptive Gaussian Mutation 
    ...
    # Adaptive Gaussian Mutation component
    adaptive_sigma = np.std(spk_pop, axis=0)
    adaptive_gaussian_perturbation = np.random.normal(0, adaptive_sigma, size=idv.shape)

    combined_perturbation = (0.15 * gaussian_perturbation + 0.15 * uniform_perturbation + 
                             0.15 * levy_perturbation + 0.15 * de_perturbation + 
                             0.15 * sa_perturbation + 0.1 * swarm_influence + 
                             0.15 * adaptive_gaussian_perturbation)
