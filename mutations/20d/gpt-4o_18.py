...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    # Combination of Gaussian, Uniform, Levy flight, Differential Evolution (DE) perturbations, Simulated Annealing (SA), Swarm Intelligence, and Chaos Theory
    ...
    chaos_perturbation = np.random.uniform(-1, 1, size=idv.shape) * np.sin(2 * np.pi * np.random.rand(*idv.shape))

    combined_perturbation = 0.15 * gaussian_perturbation + 0.15 * uniform_perturbation + 0.15 * levy_perturbation + 0.15 * de_perturbation + 0.15 * sa_perturbation + 0.15 * swarm_influence + 0.1 * chaos_perturbation
