...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    diff3 = np.mean(spk_pop, axis=0) - idv
    ...
    levy = np.random.standard_cauchy(dim) * 0.05 * np.exp(-0.01 * self.cur_gen)
    mutant += levy
    
    # Adaptive polynomial mutation
    pm_prob = max(0.1, 0.3 * np.exp(-0.005 * self.cur_gen))
    eta = 20 + 10 * (1 - np.exp(-0.01 * self.cur_gen))
    for j in range(dim):
    ...
    reset_prob = 0.02 * np.exp(-0.01 * self.cur_gen)
    ...
    cr = 0.8 * (1 - 0.5 * self.cur_gen / self.max_gen)
