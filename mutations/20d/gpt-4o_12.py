...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    # Combination of Gaussian, Uniform, Levy flight, Differential Evolution (DE) perturbations, Simulated Annealing (SA), Swarm Intelligence, and a Random Search Component
    ...
    random_search = np.random.uniform(self.lb, self.ub, size=idv.shape)

    combined_perturbation = 0.2 * gaussian_perturbation + 0.2 * uniform_perturbation + 0.2 * levy_perturbation + 0.15 * de_perturbation + 0.1 * sa_perturbation + 0.1 * swarm_influence + 0.05 * random_search
