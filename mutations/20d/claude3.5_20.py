...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    # Dimension-wise adaptive step size
    step_sizes = np.abs(spk_pop[best_idx] - idv) * np.exp(-0.01 * self.cur_gen)
    mutant += np.random.normal(0, step_sizes)
    
    # Periodic reinitialization
    if self.cur_gen % 50 == 0 and np.random.random() < 0.1:
