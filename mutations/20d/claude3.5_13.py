...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    diff3 = np.median(spk_pop[sorted_indices[:5]], axis=0) - idv
    ...
    pm_prob = max(0.1, 0.3 * np.exp(-0.007 * self.cur_gen))
    eta = 20 + 20 * (1 - np.exp(-0.015 * self.cur_gen))
    for j in range(dim):
    ...
    reset_prob = 0.02 * np.exp(-0.02 * self.cur_gen)
    ...
    if np.random.random() < 0.25:
        local_step = np.random.normal(0, 0.01 * np.exp(-0.007 * self.cur_gen), dim)
        mutant = spk_pop[best_idx] + local_step * (self.ub - self.lb)
    
    # Crossover with the best solution
    cr = 0.8 * (1 - 0.25 * self.cur_gen / self.max_gen)
    ...
    if np.random.random() < 0.3:
        ...
        neighborhood_center = np.median(spk_pop[neighbors], axis=0)
    ...
    if np.random.random() < 0.2:
        gaussian_noise = np.random.normal(0, 0.05, dim)
