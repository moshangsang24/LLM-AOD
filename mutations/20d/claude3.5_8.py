...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    cr = 0.8 - 0.3 * (self.cur_gen / self.max_gen)
    ...
    if np.random.random() < 0.2:
        ...
        search_radius = np.linalg.norm(mutant - neighborhood_center) * np.exp(-0.01 * self.cur_gen)
        mutant = neighborhood_center + np.random.normal(0, search_radius, dim)
