...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    levy = np.random.standard_cauchy(dim) * 0.05 * np.exp(-0.025 * self.cur_gen)
    mutant += levy
    
    # Adaptive polynomial mutation
    pm_prob = max(0.1, 0.3 * np.exp(-0.006 * self.cur_gen))
    eta = 20 + 20 * (1 - np.exp(-0.012 * self.cur_gen))
    for j in range(dim):
        if np.random.random() < pm_prob:
            ...
            mutant[j] += delta * (self.ub - self.lb) * 0.12
    
    # Occasional reset of dimensions
    reset_prob = 0.02 * np.exp(-0.018 * self.cur_gen)
    ...
    if np.random.random() < 0.22:
        local_step = np.random.normal(0, 0.01 * np.exp(-0.006 * self.cur_gen), dim)
        mutant = spk_pop[best_idx] + local_step * (self.ub - self.lb)
    
    # Crossover with the best solution
    cr = 0.85 * (1 - 0.28 * self.cur_gen / self.max_gen)
    ...
    if np.random.random() < 0.27:
        ...
        mutant = neighborhood_center + np.random.normal(0, 0.12, dim) * (self.ub - self.lb)
    ...
    if np.random.random() < 0.17:
        gaussian_noise = np.random.normal(0, 0.12, dim)
    ...
    if self.cur_gen % 45 == 0 and np.random.random() < 0.12:
        mutant = np.random.uniform(self.lb, self.ub, dim)
