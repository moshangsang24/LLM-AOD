...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    eta = 20 + 20 * (1 - np.exp(-0.01 * self.cur_gen))
    for j in range(dim):
    ...
    if np.random.random() < 0.2:
        gaussian_noise = np.random.normal(0, 0.1 * (1 - self.cur_gen / self.max_gen), dim)
        mutant += gaussian_noise * (self.ub - self.lb)
    
    # Uniform mutation
    if np.random.random() < 0.1:
        uniform_noise = np.random.uniform(-0.1, 0.1, dim)
        mutant += uniform_noise * (self.ub - self.lb)
