...
def mutate_operation(self, idv, spk_pop, spk_fit):
    ...
    # Combination of Gaussian, Uniform, Levy flight, Differential Evolution (DE) perturbations, Simulated Annealing (SA), Swarm Intelligence, and Particle Swarm Optimization (PSO)
    ...
    r1 = np.random.rand()
    r2 = np.random.rand()
    velocity = w * (np.random.rand(*idv.shape)) + c1 * r1 * (best_individual - idv) + c2 * r2 * (swarm_influence)

    combined_perturbation = 0.15 * gaussian_perturbation + 0.15 * uniform_perturbation + 0.15 * levy_perturbation + 0.15 * de_perturbation + 0.15 * sa_perturbation + 0.15 * swarm_influence + 0.1 * velocity
